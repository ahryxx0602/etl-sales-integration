  ...
  async (msg) => {
    ...
    try {
      ...
      if (body.type === 'csv_file') {
        // stream file, push từng dòng dạng '_row'
        const parser = fs
      ...
      if (body.type === '_row') {
        ...
        await publish(ch, CFG.ROUTING.TRANSFORM, parsed);
        ch.ack(msg);
        console.log('[validate] _row OK → transform:', body.order_id);
        return;
      }
...
//console.log('[validate] file/_row OK → transform:', (body.type==='_row'?body.order_id:body.path));
