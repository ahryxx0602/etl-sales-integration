...
async function run() {
  ...
  await ch.consume(CFG.QUEUES.TRANSFORM, async (msg) => {
    ...
    try {
      ...
      if (amtVal == null) {
        log.warn({ msg: raw }, 'missing amount -> nack to DLQ');
      ...
      if (added === 0) {
        log.info({ orderId, sku: r.product_code_clean }, 'duplicate (redis) → skip');
        ch.ack(msg);
        return;
      }
      ...
      ch.ack(msg);
      log.info({ line: r.order_line_id }, 'processed → CLEAN');

    } catch (err) {
      log.error({ err: String(err) }, 'processing failed → DLQ');
      ch.nack(msg, false, false);
    }
  }, { noAck: false });
